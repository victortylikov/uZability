	...
	<div class="main_part clearfix">
		<article class="grid_18 alpha">
			...
			<section class="section_article" id="post1">
				<header>
					<h2><a class="a_section_link" href="/spring/articles/01_uxpeople">${article.articleName}</a></h2>
				...
				<p class="section_new">
				Минске (конференц-зал гостиницы «Виктория», пр-т Победителей 59) пройдет конференция UXPeople,
			...
			<section class="section_article" id="post2">
				...
				<p class="section_new">
				По роду моей деятельности ко мне часто обращаются иностранные компании за консультациями, аналитикой и другой помощью
				в сфере китайского интернета. Нередко дело доходит до локализации сайта или приложения, тогда же встает вопрос об отдельном
			...
			<section class="section_article" id="post3">
				...
				<p class="section_new">
				Кто-то говорит, что плоский дизайн интерфейсов пришел или, точнее, объявил о себе во всей красе благодаря платформе Windows Phone 
				(и Windows 8) с ее революционными стилистически простыми и намеренно упрощенными интерфейсами. Это было ново и свежо на фоне
				повального увлечения скеуморфными, натуралистичными иконками в мире Apple. Это было интересно и необычно, потому что как минимум резко и необратимо решил поменяться и обновиться такой большой и, казалось,
				неповоротливый монстр как Microsoft.
				<a class="new_read" href="/spring/articles/03_flatdesign">Читать дальше</a>
				</p>
			</section>
			<section class="section_article" id="post4">
				...
				<p class="section_new">
				Существует множество пословиц, подразумевающих (или прямо заявляющих), что красота поверхностна 
				и ограничивается тем, что с ее помощью можно достичь. «Красота без разума пуста», «встречают по одежке, 
				провожают по уму» — лишь некоторые из них. Поскольку в индустрии веб-дизайна таланты сегодня встречаются
				сплошь и рядом, и практически кто угодно может создать «красивый» сайт, то становится все сложнее определить
				подлинные критерии красоты в веб-дизайне.
				<a class="new_read" href="/spring/articles/04_beautyindesign">Читать дальше</a>
				</p>
			</section>
